...
def test_accuracy():
    ...
    for domain_name in domains:
        ...
        predicted_attributes_map = pd.read_csv(f"{parent_folder}/{domain_name}//predicted_attributes_map.csv")

        predicted_relationships_map = pd.read_csv(f"{parent_folder}/{domain_name}//predicted_relationship_map_after_modification.csv")
        print("GPT results -----")
        # We already have actual and generated description mapped in csv files, so we will only run prompts
        workflow = WorkflowStart(predicted_attributes_map, predicted_relationships_map, domain_name)
        workflow.run()
