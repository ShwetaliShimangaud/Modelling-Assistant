...
parent_folder = f"../25th may inheritance results"

prompts = [
    "Do the following two statements semantically equal?",
    "Do the following two statements convey the exact same information?",
    "DO these two statements conveying the same meaning?",
    "Do these statements synonymous?",
...
for domain_name in domains:
    ...
    for i, row in relationship_true_result.iterrows():
        ...
        if yes_count > no_count and yes_count > unclear_count:
            relationship_map.loc[len(relationship_map)] = [generated_description,
                                                           actual_description, row['equal'], True]
        elif no_count > yes_count and no_count > unclear_count:
            relationship_map.loc[len(relationship_map)] = [generated_description,
                                                           actual_description, row['equal'], False]
        else:
            relationship_map.loc[len(relationship_map)] = [generated_description,
                                                           actual_description, row['equal'], "not clear"]

    domain_result.to_csv(f"{parent_folder}//{domain_name}_equality_check.csv")
...
relationship_map.to_csv(f"{parent_folder}/equality-prompts-results-inheritance.csv")
